...
urlpatterns = [
        ...
        re_path(r'^search/$', views.domain_search),
        re_path(r'^d/$', RedirectView.as_view(url='/')),

        re_path(r'^d/(?P<name>%s)/$' % dns_name, never_cache(views.DomainNameDetailView.as_view())),
        re_path(r'^d/(?P<name>%s)/(?P<url_subdir>dnssec/)$' % dns_name, never_cache(
            views.DomainNameDNSSECPageView.as_view())),
        re_path(r'^d/(?P<name>%s)/(?P<url_subdir>servers/)$' % dns_name, never_cache(
            views.DomainNameServersView.as_view())),
        re_path(r'^d/(?P<name>%s)/(?P<url_subdir>responses/)$' % dns_name, never_cache(
            views.DomainNameResponsesView.as_view())),
        re_path(r'^d/(?P<name>%s)/(?P<url_subdir>dnssec/)(?P<url_file>auth_graph)\.(?P<format>png|jpg|svg|dot|js)$' % dns_name, never_cache(
            views.DomainNameDNSSECGraphView.as_view())),
        re_path(r'^d/(?P<name>%s)/(?P<url_subdir>REST/)(?P<rest_dir>(raw|processed|meta)/)$' % dns_name, never_cache(
            views.DomainNameRESTView.as_view())),
        ...
        #re_path(r'^util/dnslookingglass.jnlp$', views.lookingGlassJNLP),

        re_path(r'^contact/$', views.contact),
        re_path(r'^message_submitted/$', TemplateView.as_view(template_name='message_submitted.html')),

        re_path(r'^doc/$', TemplateView.as_view(template_name='doc.html')),
        re_path(r'^doc/faq/$', TemplateView.as_view(template_name='faq.html')),
        re_path(r'^doc/dnssec/$', TemplateView.as_view(template_name='dnssec_legend.html')),
]
